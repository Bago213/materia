...
The RPM spec file provided here is for Materia-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://materia.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Materia git repository](https://github.com/materia/materia) but are not part of
...
Currently only `Source10` is used, for the example `materia.conf` file.
...
in the materia git repository. At present only an SVG version of the Materia
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the materia
...
needed when 0.12.1 is released, a proper fix is already in the Materia git
master branch.
...
idea to build Materia Core against a static version of that library compiled
...
The `materia.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `materia-server` package. At this point, most RPM based
...
The `materia-server` package should have SELinux support. How to properly do
...
Materia-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/materia/). He has been
packaging Materia for Fedora at least since 2012.
...
4. I renamed the `materia` package that contains the Qt GUI to `materia-core` as
that appears to be how the general population refers to it, in contrast to
`materia-xt` or `materia-classic`. I wanted to make sure the general population
