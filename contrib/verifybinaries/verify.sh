...
###   This script attempts to download the signature file SHA256SUMS.asc from materia.org
...
WORKINGDIR="/tmp/materia"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://materia.org/bin/"
VERSIONPREFIX="materia-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'materia-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Materia Core binary release signing key installed?"
   fi
