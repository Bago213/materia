...
*Setup instructions for a Gitian build of Materia Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Materia
...
- [Building Materia Core](#building-materia-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-materia-host/README.md).
...
Clone the git repositories for materia and Gitian.
...
in the materia repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Materia Core
----------------

To build Materia Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the materia repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/materia/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/materia-project/materia
...
For example:
```bash
URL=https://github.com/thrasher-/materia.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit materia=${COMMIT} --url materia=${URL} ../materia/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit materia=${COMMIT} --url materia=${URL} ../materia/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit materia=${COMMIT} --url materia=${URL} ../materia/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the materia git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../materia/contrib/gitian-descriptors/*|sort|uniq )
...
MATERPATH=/some/root/path/materia
SIGPATH=/some/root/path/materia-detached-sigs

./bin/gbuild --url materia=${MATERPATH},signature=${SIGPATH} ../materia/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
in `gitian.sigs.MATER` to your signing machine and do

```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/materia-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/materia-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/materia-osx-build.assert
```
...
[gitian.sigs.MATER](https://github.com/materia-project/gitian.sigs.MATER/) repository, or if that's not possible create a pull
