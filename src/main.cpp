...
#if defined(NDEBUG)
# error "Materia cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Materia Signed Message:\n";
                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s MATER additional fees, %d delta bytes\n",
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, CBlockIndex * const pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Materia: Reject block.nVersion=1 blocks (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
    ...
    if (block.nVersion < 2) {
        if (consensusParams.BIP34Height != -1) {
            ...
            // Regtest and Unittest: use Materia's supermajority rule
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Materia: (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
                     ...
                     ++mi)
                {
                    ...
                    else if (!fMissingInputs2)
                    {
                        ...
                        if (orphanTx.wit.IsNull() && !stateDummy.CorruptionPossible()) {
                            ...
                            // See https://github.com/materia/materia/issues/8279 for details.
                            assert(recentRejects);
