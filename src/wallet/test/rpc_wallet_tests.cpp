...
BOOST_AUTO_TEST_CASE(rpc_wallet)
{
    ...
    UniValue r;
    CPubKey demoPubkey;
    CMateriaAddress demoAddress;
    UniValue retValue;
    string strAccount = "walletDemoAccount";
    CMateriaAddress setaccountDemoAddress;
    {
        LOCK(pwalletMain->cs_wallet);

        demoPubkey = pwalletMain->GenerateNewKey();
        demoAddress = CMateriaAddress(CTxDestination(demoPubkey.GetID()));
        ...
        setaccountDemoAddress = CMateriaAddress(CTxDestination(setaccountDemoPubkey.GetID()));
    }
    /*********************************
     * 			setaccount
     *********************************/
    ...
    BOOST_CHECK_THROW(CallRPC("setaccount LEr4hNAefWYhBMgxCFP2Po1NPrUeiK8kM2 nullaccount"), runtime_error);
    BOOST_CHECK_THROW(CallRPC("setaccount"), runtime_error);
    /* 1D1ZrZNe3JUo7ZycKEYQQiQAWd9y54F4X (33 chars) is an illegal address (should be 34 chars) */
    BOOST_CHECK_THROW(CallRPC("setaccount LEr4hNAefWYhBMgxCFP2Po1NPrUeiK8kM nullaccount"), runtime_error);
    ...
    BOOST_CHECK(CMateriaAddress(retValue.get_str()).Get() == demoAddress.Get());
    ...
    /* wrong address */
    BOOST_CHECK(CallRPC("verifymessage LYKr1oaPsQSHthukMLDhdzsQUjgZvNQiaq " + retValue.get_str() + " mymessage").get_bool() == false);
    ...
    BOOST_CHECK(CMateriaAddress(arr[0].get_str()).Get() == demoAddress.Get());
